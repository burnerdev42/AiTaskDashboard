...
1. **Source of Truth:** The file `backend/requirement/Data requirements.txt` is the absolute **Source of Truth** for all fields, typings, derived fields, and enumerations.
2. **Ambiguity means Stop:** If the requirements document contains new fields that lack clear types or seem contradictory, **YOU MUST stop and ask the user clarifying questions.** Do not guess complex relationships.
3. **Location:** The output must ALWAYS be written to `backend/requirement/api/docs/swagger.yaml`.

---

## 🏗️ Swagger Architecture Rules
...
Unless explicitly defined otherwise in the DB, all keys in the Swagger schema properties MUST be in standard JSON `camelCase` (e.g., `Upvote Count` becomes `upvoteCount`). The `Data requirements.txt` file is structured to help you map these perfectly.

### 5. Explicit Endpoint Parity
Check the "Additional Functional Requirement" section in `Data requirements.txt`. If an endpoint is called out specifically (like `PATCH /challenges/{virtualId}/status`), it MUST exist as a distinct path in the `swagger.yaml`.

---

## 🚀 Execution Workflow
When asked to "Update the Swagger" or "Regenerate Swagger":
1. Read the `Data requirements.txt` file entirely.
2. Scan for added/removed DB fields and Derived fields.
