# Challenge API Implementation Specification — v4

> **Source:** `Data requirements.txt`, `challenge_model_spec.md`, `swagger.yaml`
> **Replaces:** `challenge_api_spec_final_v3.md`
...
### `GET /challenges/status/{status}` (New in v4)
1. Find challenges by `status` field (e.g., `completed`).
2. Support optional `limit` and `offset` for pagination.
3. Compute and attach all derived fields as in `GET /challenges`.
4. Return array of `ChallengeResponse`.
...
1. Find challenge by `virtualId`. 404 if not found.
2. Validate the incoming `status` value against `SWIM_LANE_STATUS` enum (`submitted`, `ideation`, `pilot`, `completed`, `archive`). Return 400 if invalid.
3. **Pilot-lock constraint:** If the challenge's current `status` is `pilot`, reject any transition to `submitted` with HTTP 400 and message: `"Transition from 'pilot' back to 'submitted' is not allowed."`. All other transitions from `pilot` remain permitted.
4. Update `status` field.
5. If new status is `pilot`, set `timestampOfStatusChangedToPilot = new Date()`.
6. If new status is `completed`, set `timestampOfCompleted = new Date()`.
7. Create Activity: `{ type: 'challenge_status_update', fk_id, userId }`.
8. Create Notification for all subscribers (except initiator): `{ type: 'challenge_status_update' }`.
9. Return 200.
