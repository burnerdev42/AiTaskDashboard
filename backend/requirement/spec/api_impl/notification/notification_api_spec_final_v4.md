...
2. For each notification, compute and attach derived fields:
   ...
   - `linkedEntityDetails`: Object containing `virtualId` (e.g., CH-001 or ID-0001), a `type` discriminator ('CH' or 'ID'), and optionally `challengeVirtualId` (the parent challenge's ID, required for ideas) dynamically looked up via Mongo `_id` resolving `fk_id` against Challenges or Ideas collections.
