...
import { Challenge, ChallengeDocument } from '@app/models/challenges/challenge.schema';
...
import { Activity, ActivityDocument } from '@app/models/activities/activity.schema';
import { Notification, NotificationDocument } from '@app/models/notifications/notification.schema';
...
export class SeedService {
    ...
    private async createUsers(): Promise<UserDocument[]> {
        ...
        for (const user of createdUsers) {
            await this.logActivity('log_in', undefined, (user._id as any).toString());
        }

        return createdUsers as UserDocument[];
    }

    private async createChallenges(users: UserDocument[]): Promise<ChallengeDocument[]> {
        ...
        for (const challenge of createdChallenges) {
            await this.logActivity('challenge_created', challenge._id.toString(), challenge.userId);
            await this.notifyAllUsers('challenge_created', challenge._id.toString(), challenge.userId, users);
        }

        return createdChallenges as ChallengeDocument[];
    }

    private async createIdeas(challenges: ChallengeDocument[], users: UserDocument[]): Promise<IdeaDocument[]> {
        const ideas: any[] = [];
        let ideaCounter = 1;
        ...
        for (const idea of createdIdeas) {
            ...
            await this.notifySubscribersAndOwner('idea_created', idea._id.toString(), idea.userId, idea.challengeId);
        }

        return createdIdeas as IdeaDocument[];
    }

    private async createComments(challenges: ChallengeDocument[], ideas: IdeaDocument[], users: UserDocument[]) {
        ...
        await this.logActivity('challenge_commented', chComment._id.toString(), chCommenter._id.toString());
        await this.notifySubscribersAndOwner('challenge_commented', chComment._id.toString(), chCommenter._id.toString(), challenge._id.toString());
        ...
        await this.logActivity('idea_commented', idComment._id.toString(), idCommenter._id.toString());
        await this.notifySubscribersAndOwner('idea_commented', idComment._id.toString(), idCommenter._id.toString(), idea._id.toString(), true);
    }

    private async logActivity(type: string, fk_id: string | undefined, userId: string) {
    ...
    private async notifyAllUsers(type: string, fk_id: string, initiatorId: string, users: UserDocument[]) {
    ...
    private async notifySubscribersAndOwner(type: string, fk_id: string, initiatorId: string, entityId: string, isIdea: boolean = false) {
        ...
        if (isIdea) {
            ...
            if (idea) {
                ...
                const challenge = await this.challengeModel.findById(idea.challengeId);
                if (challenge) {
                    subscriberIds = [...new Set([...subscriberIds, ...challenge.subcriptions])];
        ...
        const recipientIds = [...new Set([...subscriberIds, ownerId])].filter(id => id !== initiatorId);
