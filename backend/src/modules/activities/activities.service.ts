...
export class ActivitiesService extends AbstractService {
    ...
    /** Update an activity. */
    async update(id: string, updates: Partial<CreateActivityDto>): Promise<ActivityDocument> {
    ...
    async findRecentSubmissions(userId: string, limit = 5): Promise<ActivityDocument[]> {
        return this.activityModel
            .find({
                userId,
